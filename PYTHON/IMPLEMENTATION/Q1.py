...
# 경우의 수를 출력하는 프로그램을 작성하시오. 이때 왕실의 정원에서 행 위치를 표현할 때는
# 1로부터 8로 표현하며, 열 위치를 표현할 때는 a부터 h로 표현한다.
# 입력 조건
# - 첫째 줄에 8 X 8 좌표 평면상에서 현재 나이트가 위치한 곳의 좌표를 나타내는 ㅁ두 문자로 구성된 문자열이 입력된다.
