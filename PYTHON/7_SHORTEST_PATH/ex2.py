...
while q :
    ...
    if (dist > d[now]) :
        continue
    for i in graph[now] :
        this, thisDist = i[0], i[1]
        cost = thisDist + dist
        if (d[this] > cost) :
            d[this] = cost
            heapq.heappush(q, (cost, this))
...
# 문제 해설
# 이 문제는 한 도시에서 다른 도시까지의 최단 거리 문제로 치환할 수 있으므로 다익스트라 알고리즘을 이용해서 풀 수 있다.
# 또한 N과 M의 범위가 충분히 크기 때문에, 우선순위 큐를 이용하여 다익스트라 알고리즘을 작성해야 한다.
# 다루었던 다익스트라 알고리즘의 소스코드에서 마지막 부분만 조금 수정하여 답안 코드를 만들 수 있다.
